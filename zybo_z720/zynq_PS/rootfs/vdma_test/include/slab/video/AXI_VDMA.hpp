...
public:
	...
	AXI_VDMA(uint16_t dev_id, uint32_t frame_write_buf_base_addr, uint32_t frame_read_buf_base_addr, IrptCtl& irpt_ctl, uint16_t rd_irpt_id, uint16_t wr_irpt_id) :
		...
		context_{},
		frame_write_buf_base_addr_(frame_write_buf_base_addr),
		frame_read_buf_base_addr_(frame_read_buf_base_addr),
		irpt_ctl_(irpt_ctl)
	{
...
//		while (XAxiVdma_ChannelIsRunning(&drv_inst_.WriteChannel)) ;
	...
	void configureRead(uint16_t h_res, uint16_t v_res)
	{
		...
		uint32_t addr = frame_read_buf_base_addr_;
		...
		if (XST_SUCCESS != status)
		{
			std::cout << "error : XAxiVdma_DmaSetBufferAddr()" << std::endl;
			throw std::runtime_error(__FILE__ ":" LINE_STRING);
		}
	...
	void configureWrite(uint16_t h_res, uint16_t v_res)
	{
		...
		uint32_t addr = frame_write_buf_base_addr_;
		for (int iFrm=0; iFrm<drv_inst_.MaxNumFrames; ++iFrm) {
...
private:
	...
	uint32_t frame_read_buf_base_addr_;
	uint32_t frame_write_buf_base_addr_;
	IrptCtl& irpt_ctl_;
