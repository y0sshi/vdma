...
namespace slab {
	VDMA::VDMA(uint32_t base_addr, Resolution res) :
	...
	void VDMA::map_framebuffer() {
		...
		map_mem_ = (bgr_t*)mmap(0, frameBytes_, PROT_READ | PROT_WRITE, MAP_SHARED, fd_, base_addr_ & ~MAP_MASK);
		frame_buf_ = map_mem_;
	}

	void VDMA::unmap_framebuffer() {
		/* close memory */
		if(munmap(map_mem_, frameBytes_) == -1) FATAL;
		close(fd_);
	}

	void VDMA::set_framebuffer(const bgr_t* img, const uint8_t frame_index) {
		memcpy(frame_buf_ + (pixels_ * frame_index), img, frameBytes_);
	}
